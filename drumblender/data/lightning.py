...
from drumblender.data.collate import pad_audio_params_collate # collate -xXx
from drumblender.data.bucketing import BucketingBatchSampler # bucketing -xXx
...
class AudioDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        batch_size: int = 32,
        num_workers: int = 0,
        dataset_class: Type[TorchDataset] = AudioDataset,
        ...
        sample_rate=48000,
        num_samples: Optional[int] = 48000 * 2, # - None 허용 xXx
        use_bucketing: bool = True, # xXx
        ...
        self.use_bucketing = use_bucketing # xXx
    ...
    def setup(self, stage: str):
        ...
        if stage in ("fit", None):
            ...
            # --- xXx: bucketing용 lengths 캐시(없으면 생성) ---
            if self.use_bucketing and not hasattr(self.train_dataset, "lengths"):
                try:
                    lengths = []
                    ...
                    self.train_dataset.lengths = None
        ###
        elif stage == "validate":
            self.val_dataset = self.dataset_cls(
                *args,
    ...
    def train_dataloader(self):
        ...
        # fallback: 일반 shuffle + collate (버킷팅 없으면 padding이 커질 수 있음)
        return DataLoader(
            self.train_dataset,
            batch_size=self.batch_size,
            num_workers=self.num_workers,
...
class ModalDataModule(AudioDataModule):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            use_bucketing=use_bucketing, # xXx
    ...
    def preprocess_dataset(self) -> None:
        ...
        for key, item in tqdm(metadata.items()):
            ...
            ###
            if self.save_modal_audio:
                # 가변 길이면 원본 waveform 길이를 사용
                num_samples_for_this = (
                    waveform.shape[-1] if self.num_samples is None else self.num_samples
                )
                ...
                # Update the metadata (modal 파일 경로는 save_modal_audio일 때만 존재)
