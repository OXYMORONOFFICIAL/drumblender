...
class DrumBlender(pl.LightningModule):
    ...
    ###
    @staticmethod
    def _make_time_mask(lengths: torch.Tensor, T: int, device) -> torch.Tensor:
        # returns [B,1,T]
    ...
    def forward(
        ...
        """
        original: [B,1,Tmax] (padded)
        params:   whatever the modal_synth / autoencoder expects
        lengths:  [B] true lengths (optional)
        """
        ...
        def _pad_to_Tmax(x: torch.Tensor, T: int) -> torch.Tensor:
            # expects [B,1,L] or [B,L]
            if x.dim() == 2:
                x = x.unsqueeze(1)
            L = x.shape[-1]
            if L == T:
        ...
        # Variable-length path: per-sample modal/noise, then transient once
        ys = []
    ...
    def _do_step(self, batch):
        ...
        y_hat = self(original, params, lengths=lengths)  # forward도 lengths 받게(아래 3번)
        if lengths is not None:
            T = original.shape[-1]
